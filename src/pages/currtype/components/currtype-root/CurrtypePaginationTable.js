...
import './index.less'
export default class CurrtypePaginationTable extends Component {
    constructor(props){
        super(props);
        let self=this;
        this.state = {
    ...
    componentDidMount(){
        ...
        actions.currtype.loadList();//table数据
    }
    ...
    cellClick = async (record,btnFlag) => {
        await actions.currtype.updateState({
            rowData : record,
        });
    ...
    // 流程提交成功后回调函数
    onSubmitSuc = async ()=>{
        await actions.currtype.loadList();
        actions.currtype.updateState({showLoading:false});
    ...
    // 提交操作初始执行操作
    onSubmitStart = ()=>{
        actions.currtype.updateState({showLoading:true});
    ...
    onExamine = async (text, record, index)=> {
        ...
        await actions.currtype.updateState({rowData:record});
    ...
    onModalDel = async (delFlag)=>{
        ...
        if(delFlag){
            await actions.currtype.delItem({
                param: delData
            });
        }
        this.setState({
