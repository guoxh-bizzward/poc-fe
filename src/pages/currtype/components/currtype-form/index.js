...
class CurrtypeForm extends Component {
    constructor(props){
        super(props)
        this.state = {
            code: '',
    ...
    componentWillMount(){
        // 获得币种列表数据
        actions.currtype.getOrderTypes();
    }
    /** 查询数据
     * @param {*} error 校验是否成功
     * @param {*} values 表单数据
    ...
    search = (error,values) => {
        this.props.form.validateFields(async (err, values) => {
            ...
            await actions.currtype.loadList(values);
        });
...
export default Form.createForm()(CurrtypeForm)